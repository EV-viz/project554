d3.csv("../data/EV_Population_Non_Zero_Range.csv").then((data) => {
  ...
  function calculateBins(dataset) {
    let histogram = d3
      ...
      .thresholds(
        d3.range(
          0,
          d3.max(dataset, (d) => d.electricRange),
          10
        )
      );
  ...
  function updateHistogram(filterType) {
    ...
    let y = d3.scaleLinear().domain([0, overallMaxBinCount]).range([height, 0]);
  ...
  d3.select("#PHEVButton").on("click", () =>
    updateHistogram("Plug-in Hybrid Electric Vehicle (PHEV)")
  );
  d3.select("#BEVButton").on("click", () =>
    updateHistogram("Battery Electric Vehicle (BEV)")
  );
